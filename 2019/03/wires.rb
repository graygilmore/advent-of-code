...
class PartOne
  ...
  def solution
    overlapping_wire_positions.map do |position, _wires|
  ...
  def overlapping_wire_positions
    grid = { [0, 0] => {} }
    ...
    grid.select { |key, wires| wires.length > 1 }
  end
end

class PartTwo < PartOne
  def solution
    overlapping_wire_positions.map do |_position, wires|
      wires.values.inject(:+)
    end.sort.first
  end
end
